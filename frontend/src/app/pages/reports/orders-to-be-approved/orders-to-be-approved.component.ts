...
import { OrderReportPrintComponent } from '../orders-report/print/order-report-print.component';
                      ...
                      <div class="flex flex-col gap-1.5">
                        <label class="text-[10px] font-bold text-surface-400 uppercase">
                          ...
                          <span class="text-surface-900 dark:text-white font-bold">{{
                            product.amount * product.qty | number: '1.2-2'
                          }}</span>
                        </label>
                      </div>
...
export class OrdersToBeApproved implements OnInit {
  ...
  // Edit sayfası canlı toplamları
  editTotals = computed(() => {
    const order = this.selectedOrder();
    if (!order) return { subTotal: 0, discountAmount: 0, grandTotal: 0, totalQty: 0 };

    // Mevcut (backend'den gelen) toplamlar
    let subTotal = order.subTotal || order.total + order.discount;
  ...
  ngOnInit() {
    this.loadLookups();
    this.onReport(); // Otomatik olarak verileri yükle
  }
