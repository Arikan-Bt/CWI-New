...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ButtonModule, TagModule, CurrencyPipe, InputNumberModule, FormsModule],
  template: `
    <div
      class="group bg-white dark:bg-surface-900 border border-surface-200 dark:border-surface-700 rounded-2xl overflow-hidden hover:shadow-xl dark:hover:shadow-primary-900/10 transition-all duration-300 h-full flex flex-col relative z-0"
    >
      <!-- Stok Durumu -->
      <div class="absolute top-3 right-3 z-10">
        @if (product().isInStock) {
          <p-tag
            ...
            severity="success"
            [rounded]="true"
            styleClass="px-2 py-1 text-[10px] font-bold shadow-sm"
          ></p-tag>
        } @else {
          <p-tag
            value="Out of Stock"
      ...
      <!-- Görsel Alanı -->
      <div
        class="relative aspect-4/5 bg-surface-50 dark:bg-surface-950/50 flex items-center justify-center p-6 overflow-hidden group-hover:bg-surface-100 dark:group-hover:bg-surface-900/50 transition-colors duration-300"
      >
        @if (!imageUrl()) {
          <div
      ...
      <div class="p-4 flex flex-col flex-1 gap-3 relative bg-white dark:bg-surface-900">
        <div class="flex-1">
          <h3
            class="font-bold text-surface-900 dark:text-surface-50 text-sm leading-tight mb-1 line-clamp-2 min-h-10"
            [title]="product().name"
          >
            {{ product().name }}
          </h3>
          <span
            class="text-[10px] font-bold text-surface-400 dark:text-surface-500 uppercase tracking-widest block mb-1"
          >
            {{ product().brandName }}
          </span>
          <p
            class="text-[10px] font-medium text-surface-500 dark:text-surface-400 flex items-center gap-1"
          >
          ...
          @if (product().isInStock) {
            <div (click)="$event.stopPropagation()" class="w-full">
              <p-inputNumber
                ...
                [inputStyleClass]="
                  'w-full text-center p-0 text-sm h-9 border-none bg-transparent font-medium'
                "
...
export class ProductCardComponent {
  ...
  fallbackLevel = signal(0);
  quantity = signal(1);

  imageUrl = computed(() => {
    return this.imageService.getImageUrl({
      sku: this.product().sku,
      imageUrl: this.product().imageUrl || undefined,
      imageFallback: this.fallbackLevel(),
    });
  });

  onImageError() {
    this.fallbackLevel.update((v) => v + 1);
  }
