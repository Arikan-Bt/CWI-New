...
export class AppMenuitem {
  ...
  updateActiveStateFromRoute() {
    ...
    if (this.item.routerLink) {
      activeRoute = this.router.isActive(this.item.routerLink[0], {
        paths: 'exact',
        queryParams: 'ignored',
        matrixParams: 'ignored',
        fragment: 'ignored',
    ...
    if (!activeRoute && this.item.items) {
      activeRoute = this.item.items.some((child) => this.isChildActive(child));
    }
  ...
  isChildActive(item: MenuItem): boolean {
    ...
    if (item.items) {
      return item.items.some((child) => this.isChildActive(child));
    }
    return false;
  }
